  ...
  <form method="post" action="{{ route('manage_process') }}">
    ...
    <div class="form-group">
     ...
     <div class="form-group">
        <label>Source</label>
         <select class="form-control @error('source') is-invalid @enderror" name="source">
            ...
            <option value="News" {{ ($blog['source']=='News') ? 'selected' : '' }}>News</option>
            <option value="Media" {{ ($blog['source']=='Media') ? 'selected' : '' }}>Media</option>
     ...
     <div class="form-group">
        ...
        <textarea class="form-control @error('description') is-invalid @enderror" name="description">{{ $blog['title'] }}</textarea>
        @error('description')
            <br>
            <div class="alert alert-danger">{{ $message }}</div>
        @enderror
